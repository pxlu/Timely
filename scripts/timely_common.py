...
def _init_resource_list():
	...
	resources = [Resource(name=resource['name'], rID=resource['id'], resourcetype=resource['type'],capacity=resource['capacity'], location=resource['location'], address=resource['address'] if resource['address'] != '' else None, contact=resource['contact'], services=resource['services'], cost=resource['cost'] if resource['cost'] != ''else None, availibility=None) for resource in resources_list]
